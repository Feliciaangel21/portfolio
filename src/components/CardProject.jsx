...
const CardProject = ({ Img, Title, Description, Link: ProjectLink, id, Tags = [], ActiveTag = null }) => {
    ...
    <div className="group relative w-full h-full">
      ...
      <div className="relative flex flex-col h-full overflow-hidden rounded-xl bg-gradient-to-br from-slate-900/90 to-slate-800/90 backdrop-blur-lg border border-white/10 shadow-2xl transition-all duration-300 hover:shadow-purple-500/20">
        ...
        <div className="relative flex flex-col flex-1 p-5 z-10">
          <div className="relative overflow-hidden rounded-lg aspect-video shrink-0">
            <img
              src={Img}
              alt={Title}
          ...
          <div className="mt-4 flex flex-col flex-1 space-y-3">
            {Tags.length > 0 && (
              <div className="flex flex-wrap gap-1.5">
                {Tags.map((tag) => (
                  <span
                    key={tag}
                    className={`px-2.5 py-1 rounded-md text-xs font-medium border transition-colors duration-300 ${
                      tag === ActiveTag
                        ? "text-white bg-purple-500/25 border-purple-400/50"
            ...
            <h3 className="text-xl font-semibold bg-gradient-to-r from-blue-200 via-purple-200 to-pink-200 bg-clip-text text-transparent line-clamp-2">
              {Title}
            </h3>
