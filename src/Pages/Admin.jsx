...
export default function Admin() {
  ...
  return <main className="min-h-screen bg-[#030014] px-5 py-10 text-white"><div className="mx-auto max-w-5xl space-y-7"><header className="flex items-center justify-between"><div><h1 className="text-3xl font-bold">Project images</h1><p className="text-sm text-slate-400">Signed in as {session.user.email}</p></div><button onClick={() => supabase.auth.signOut()} className="flex items-center gap-2 rounded-lg border border-white/10 px-4 py-2 hover:bg-white/5"><LogOut className="h-4 w-4" /> Sign out</button></header><section className="space-y-5 rounded-2xl border border-white/10 bg-white/[0.04] p-6"><label className="block"><span className="mb-2 block text-sm text-slate-300">Project</span><select value={projectId} onChange={(event) => setProjectId(event.target.value)} className="w-full rounded-xl border border-white/10 bg-[#111027] px-4 py-3"><option value="">Choose a project…</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.Title}</option>)}</select></label>{selectedProject && <><div className="grid grid-cols-2 gap-3 md:grid-cols-4">{images.map((image) => <div key={image} className="group relative overflow-hidden rounded-xl border border-white/10"><img src={image} alt="" className="aspect-video h-full w-full object-cover" /><button type="button" disabled={busy} onClick={() => removeImage(image)} aria-label="Remove image" className="absolute right-2 top-2 rounded-full bg-red-600 p-1.5 opacity-90 hover:opacity-100"><X className="h-4 w-4" /></button></div>)}</div><form onSubmit={upload} className="space-y-3"><input type="file" accept="image/*" multiple onChange={(event) => setFiles(Array.from(event.target.files || []))} className="block w-full text-sm text-slate-300 file:mr-4 file:rounded-lg file:border-0 file:bg-purple-500/20 file:px-4 file:py-2 file:text-purple-200" /><button disabled={busy || !files.length} className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-500 px-5 py-3 font-semibold disabled:opacity-40"><Upload className="h-4 w-4" /> {busy ? "Saving…" : `Upload ${files.length || ""} image${files.length === 1 ? "" : "s"}`}</button></form></>}{message && <p className="text-sm text-amber-300">{message}</p>}</section></div></main>;
}
